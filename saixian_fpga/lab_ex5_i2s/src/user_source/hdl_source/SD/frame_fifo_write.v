...
assign write_data_ready = ((write_len_latch <= (rdusedw + write_cnt)) ||
                           (rdusedw > BURST_SIZE));
...
// Claim the SDRAM write turn before starting a burst.  Keep the claim while a
// read burst is still active, so the read and write FSMs cannot both start on
// the same mem_clk edge.  The previous one-cycle pulse could be stale and let
// both App_rd_en and App_wr_en become active, corrupting a random image band.
always @(posedge mem_clk or posedge rst)
begin
	if(rst)
		into_burst <= 1'b0;
	else if(state != S_CHECK_FIFO)
		into_burst <= 1'b0;
	else if(!into_burst && write_data_ready)
		into_burst <= 1'b1;
end
	...
	begin
		...
		case(state)
			...
			begin
				...
				else if(into_burst && ~App_rd_busy)
				begin
					state <= S_WRITE_BURST;
